...
class Logger:
    def __init__(self, path=None, level=LoggerLevels.NONE):
        ...
        if not isinstance(path, Path):
            raise NotImplementedError(
                "The parameter path should be a string or a Path like object, {} provided.".format(type(path))
            )
    ...
    def _message(self, level, message):
        out = "{prefix} - {level} - {message}".format(prefix=self._prefix(), level=level, message=message)
